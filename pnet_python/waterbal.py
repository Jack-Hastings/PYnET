...
def waterbal(site, veg, clim, share, rstep, timestep):
    ...
    fast_flow = veg['fast_flow_frac'] * water_in
    water_in -= fast_flow
    ...
    if clim.loc[rstep, 'o3'] > 0 and share['can_grs_psn'] > 0:

        if site['wue_o3_eff'] == 0:
            # no o3 effect on wue (assumes no stomatal impairment)
            can_d_o3 = share['can_d_o3_pot'] + (1 - share['can_d_o3_pot']) * (1 - share['d_water']) # drough offset to 03 effect
        else:
            # reduce the degree to which drought offsets o3 (assume stomatal impairment in porportion to effect on psn)
            can_d_o3 = share['can_d_o3_pot'] + (1 - share['can_d_o3_pot']) * (1 - share['d_water'] / share['can_d_o3_pot'])

        # relative drought offset
        share['drought_o3_frac'] = share['can_d_o3_pot'] / can_d_o3

    else:
        can_d_o3 = 1
        share['drought_o3_frac']
    ...
    share['net_psn_mo'] = (can_grs_psn_act - (share['day_rsp'] + share['night_rsp']) * veg['fol_ms']) * share['dayspan'] * can_d_o3
